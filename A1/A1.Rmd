...
*Build four predictive models using linear regression for earnings per hour. The target variable is earnings per hour, all others would be predictors. Model 1 shall be the simplest, model 4 the more complex. It shall be OLS. You shall explain your choice of predictors.*

In order to construct 4 different models to predict earnings per hour, I first filter for the occupation '2310' which includes elementary and middle school teachers. Possible research questions could therefore be 
...
model_4 <- lm(earnh  ~ age  + lnage + edu_BA + edu_MA + edu_Prof + edu_PhD + female + union + native + private + white + married + divorced + wirowed + nevermar + child0 + + child1 + child2 + child3 + child4plus + female_age + female_lnage, data)
summary(model_4)
...
cv2 <- train(earnh  ~  lnage + edu_BA + edu_MA + edu_Prof + edu_PhD, data, method = "lm", trControl = trainControl(method = "cv", number = k))

cv3 <- train(earnh  ~ age  + lnage + edu_BA + edu_MA + edu_Prof + edu_PhD + female + union + native + private + white + married + divorced + wirowed + nevermar + child0 + + child1 + child2 + child3 + child4plus, data, method = "lm", trControl = trainControl(method = "cv", number = k), na.action = "na.omit")

cv4 <- train(earnh  ~ age  + lnage + edu_BA + edu_MA + edu_Prof + edu_PhD + female + union + native + private + white + married + divorced + wirowed + nevermar + child0 + + child1 + child2 + child3 + child4plus + female_age + female_lnage, data, method = "lm", trControl = trainControl(method = "cv", number = k), na.action = "na.omit")
...
#stargazer(cv_mat, summary = F, digits=2, float=F) 
