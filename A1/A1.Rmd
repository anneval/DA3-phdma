...
plot_ageearnh <- ggplot(data = data, aes(x=age, y=earnh)) +
...
### 5) Interactions 
```{r}
#5. Thinking interactions  - boxplots? 
...
```{r}
# data summary
datasummary(  earnh + age + agesq + lnage + female + female_age +female_agesq +female_lnage + native + private + white + union + married + divorced + wirowed + nevermar + child0 + child1 + child2 + child3 + child4plus       ~
...
### Model 1: the target variable is continous: earnings per hour
###The goal is to predict the earnings per hour that may be appropriate for an teachers fulfilling certain criteria.
```{r}
# earn ~ grade92
```
...
```{r}
# earn ~ grade92 + age + agesq + sex + sector
```
### Model 3: the target variable is continous: earnings per hour

```{r}
# earn ~ grade92+ age + agesq + stfips+ race + sex
```
### Model 4: the target variable is continous: earnings per hour

```{r}
# earn ~ grade92+ age + agesq + stfips+ race + sex + marrital + childpres + prcitship + union  INTERACTIONS(age*sex,agesq*sex)
