...
Third, we decided for a threshold of 30% annual growth. In our sample this is equivalent to classifying the fastest growing 20% of firms as HGF. In the literature, both thresholds on a certain growth rates and a certain percentile of firms are used. We decided for a threshold based on a specific growth rates since we will analyze differen sectors (manufacturing and services) with a fixed benchmark.
...
## Feature Engineering
- included some new variables that were created from other variables
- windsorized a range of variables (negative values to =0), and included flags for it
- drop some variables 
- impute some variables (including sales growth) --> defend decision to impute with 0 growth
...
plots$extra_profit_loss_rel
# plots$extra_exp_rel # does not work, neither: fixed_assets_rel
plots[[6]]
...
glm_results$extra_profit_loss_rel


```

Asset variables do not show so much variation.
...
quad <- c("intang_assets_rel", "inventories_rel", "liq_assets_rel", "material_exp_rel",
          ...
          "profit_loss_year_rel", "share_eq_rel", "total_assets", "ceo_age", 
          "age", "extra_profit_loss_rel")
...
# interactions
interactions_plot <- ggplot(bisnode, aes(x = age, y = high_growth, color = ind2)) +
...
# are below variables there?? more sales vars?
d1 <-  c("d1_sales_mil_log_mod", "d1_sales_mil_log_mod_sq",
         "flag_low_d1_sales_mil_log", "flag_high_d1_sales_mil_log")

# more labour vars?
hr <- c("ceo_old", "ceo_young", "ceo_age", "flag_missing_ceo_age", 
        "ceo_count", "gender", "multiple_ceo", "origin", # have origin instead of foreign_management
        "labor_avg", "flag_miss_labor_avg")

firm <- c("age", "age2", "new", "urban_m", "region_m", "ind", "ind2", "ind2_cat")

# test <- c(rawvars, qualityvars, engvar, engvar2, engvar3, d1, hr, firm)
# setdiff(names(bisnode), test);setdiff(test, names(bisnode))

# interactions for logit, LASSO
interactions1 <- c("ind2*age", "ind2*age2",
                   "ind2*d1_sales_mil_log_mod", "ind2*sales_mil_log", # which sales vars??
                   "ind2*ceo_age", "ind2*origin", "ind2*gender",
                   "ind2*urban_m", "ind2*labor_avg", "ind2*multiple_ceo") # which labour vars??
# which sales variables?
...
# for simple logit
# which sales vars?
logitvars <- c("sales_mil_log", "sales_mil_log_sq", qualityvars, engvar, engvar2, engvar3, d1, hr, firm)

# for RF (no interactions, no modified features)
# which sales vars???
rfvars  <-  c("sales_mil", "d1_sales_mil_log", rawvars, qualityvars, hr, firm)

# for logit LASSO:
# more sales vars??
lassovars <- c(qualityvars, engvar, engvar2, engvar3, d1, hr, firm, interactions1, interactions2)
