...
train_control <- trainControl(method = "cv",
                              ...
                              savePredictions = "all", # save the predictions for each fold
                              verboseIter = TRUE)
...
# calculate number of variables and coefficients for all models
ncoeff <- list()
ncoeff[["logit_no_interactions"]] <- length(logit_models[["logit_no_interactions"]]$coefnames)
ncoeff[["logit_incl_interactions"]] <- length(logit_models[["logit_incl_interactions"]]$coefnames)
ncoeff[["LASSO"]] <- length(logit_models[["LASSO"]]$coefnames)
ncoeff[["rf_p"]] <- length(rf_model_p$coefnames)

# calculate number of variables for all models
##### OPEN TODO: include number of variables (have to unselect quadratics and interactions for logit)
##### OPEN: make nice row_names names

summary_all_mods <- data.frame("Coefficients" = unlist(ncoeff),
...
kable(x = summary_all_mods, format = "latex", booktabs=TRUE,  digits = 3, row.names = TRUE,
      linesep = "", col.names = c("Coefficients", "CV RMSE", "CV AUC",
...
ind2_varnames <- grep("ind2",varnames, value = TRUE) # change
ind2_cat_varnames <- grep("ind2_cat",varnames, value = TRUE)# change 
...
rf_model_unified <- ranger.unify(rf_model_p_manu$finalModel, bisnode_holdout)
