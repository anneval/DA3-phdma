...
performs better in the services sector. Further results and interactive graphs can be found in the shiny app linked to this report.
...
# set output directory (to github folder)
output <- "output/"
```
...
```{r data visualisation, include=FALSE}
# visualize sales
...
# save some plots for easier generation of summary report
# saveRDS(plot_sales_growth_both, paste0(output, "plot_sales_growth_both.RDS"))
...
shows the distribution of growth rates for both measures, where log-differences
are right-skewed. 
...
# new variables and variable transformation-------------------------------------
...
# two examples
high_growth_new <- ggplot(bisnode, aes(x = factor(new), fill = factor(high_growth))) +
...
high_growth_young <- ggplot(bisnode, aes(x = factor(ceo_young), fill = factor(high_growth))) +
  geom_bar(position = "fill", stat = "count") +
  labs(y = "Proportion", fill = "High Growth") +
  theme_classic() +
  theme(axis.title.x = element_text(size = 10),
...
# interactions
high_growth_ind2 <- ggplot(bisnode, aes(x = age, y = high_growth, color = ind2)) +
...
other, very important variables (mainly assets, CEO and industry
...
seem to affect *fast_growth* as much (see examples below).


```{r plots relation, include=TRUE, echo=FALSE, fig.align='center', fig.height=2.5, fig.width=8}
# plots to show
grid.arrange(plots$ceo_age, plots$curr_assets_rel, ncol = 2)
# saveRDS(plots$ceo_age, paste0(output, "plot_ceo_age.RDS"))
# saveRDS(plots$curr_assets_rel, paste0(output, "plot_curr_assets.RDS"))
```
...
a role. As regards possible interactions, the below right plot hints towards a
...
# saveRDS(high_growth_new, paste0(output, "high_growth_new.RDS"))
# saveRDS(high_growth_young, paste0(output, "high_growth_young.RDS"))
# saveRDS(high_growth_ind2, paste0(output, "high_growth_ind2.RDS"))
...
# due to faster rendering of plot, we load the below (and some more) models
...
# }
# 
# saveRDS(logit_models, paste0(output, "logit_models.RDS"))
# saveRDS(CV_RMSE_folds, paste0(output, "CV_RMSE_folds.RDS"))

logit_models <- readRDS(paste0(output, "logit_models.RDS"))
CV_RMSE_folds <- readRDS(paste0(output, "CV_RMSE_folds.RDS"))
...
#   importance = "impurity"
# )
# saveRDS(rf_model_p, paste0(output, "rf_model_p.RDS"))
rf_model_p <- readRDS(paste0(output, "rf_model_p.RDS"))
rf_model_p$results
...
# })
# 
# saveRDS(logit_lasso_model, paste0(output, "logit_lasso_model.RDS"))
logit_lasso_model <- readRDS(paste0(output, "logit_lasso_model.RDS"))
...
The below table shows the results for our different model specifications.
...
summary_no_loss_table <- kable(x = summary_no_loss, format = "latex", booktabs = TRUE,  digits = 3, row.names = TRUE,
      linesep = "", caption = "Summary of Prediction Models (no loss function)") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position")

summary_no_loss_table
# saveRDS(summary_no_loss_table, paste0(output, "summary_no_loss_table.RDS"))
```
...
# Take best model and estimate RMSE, AUC, expected loss on holdout--------------
...
discrete_roc_plot <- ggplot(
  ...
  theme(legend.title = element_text(size = 8), 
        ...
        axis.title.y = element_text(size = 10)) 
discrete_roc_plot
# saveRDS(discrete_roc_plot, paste0(output, "discrete_roc_plot.RDS"))

```

The confusion matrix for default classification (using 0.5 as
...
cf1_kable <- kable(x = cm1_perc, format = "latex", booktabs = TRUE,  digits = 3, row.names = TRUE,
      linesep = "", caption = "Confusion Matrix (Threshold: 0.5)") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position") # specify exact place in pdf ouput
# saveRDS(cf1_kable, paste0(output, "cf1_kable.RDS"))
...
cf2_kable <- kable(x = cm2_perc, format = "latex", booktabs = TRUE,  digits = 3, row.names = TRUE,
      linesep = "", caption = "Confusion Matrix (Threshold: 0.2)") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position")

# saveRDS(cf2_kable, paste0(output, "cf2_kable.RDS"))
...
# Now use loss function and search for best thresholds
# and expected loss over folds
best_tresholds_cv <- list()
expected_loss_cv <- list()
...
summary_all_mods_kable <- kable(x = summary_all_mods, format = "latex", booktabs=TRUE,  digits = 3, #row.names = TRUE,
      linesep = "", col.names = c("Model", "Preds", "Coeffs", "CV RMSE", "CV AUC",
                                  "CV threshold", "CV expected Loss")) %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position")

# saveRDS(summary_all_mods_kable, paste0(output, "summary_all_mods_kable.RDS"))
...
<!-- Task2.1: Random forest Manufacturing (WITHOUT LOSS FUNCTION!!!)  -->
...
#   importance = "impurity"
# )
# saveRDS(rf_model_p_manu, paste0(output, "rf_model_p_manu.RDS"))
rf_model_p_manu <- readRDS(paste0(output, "rf_model_p_manu.RDS"))
...
<!-- Task2.1: Random forest Manufacturing (INCLUDING LOSS FUNCTION!!!)  -->

```{r loss function rf manufacturing}
# ------------------------------------------------------------------------------
...
<!-- Task2.1: Random forest services (WITHOUT LOSS FUNCTION!!!)  -->
...
#   importance = "impurity"
# )
# saveRDS(rf_model_p_serv, paste0(output,"rf_model_p_serv.RDS"))
rf_model_p_serv <- readRDS(paste0(output, "rf_model_p_serv.RDS"))
...
```
<!-- Task2.1: Random forest services (INCLUDING LOSS FUNCTION!!!)  -->

```{r loss function rf services}
# ------------------------------------------------------------------------------
...
summary_manu_serv_kable <- kable(x = summary_manu_serv, format = "latex", booktabs=TRUE,  digits = 3, row.names = TRUE,
      linesep = "", col.names = c("Model", "CV RMSE", "CV AUC",
                                  "CV threshold", "CV expected Loss"), caption = "Comparing Manufacturing and Services Models") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position")
...
cm5_perc_kable <- kable(x = cm5_perc, format = "latex", booktabs = TRUE,  digits = 3, row.names = TRUE,
      linesep = "", caption = "Confusion Matrix Manufacturing") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position")

cm6_perc_kable <- kable(x = cm6_perc, format = "latex", booktabs = TRUE,  digits = 3, row.names = TRUE,
      linesep = "", caption = "Confusion Matrix Services") %>%
  kable_styling(position = "center",
                latex_options = "HOLD_position") 
...
The random forest performs better in the services sector. It exhibits a lower RMSE, a higher AUC and a lower expected loss. More differences can be seen in the respective confusion tables. 

```{r include table manu serv, echo=FALSE, include=TRUE}
# ------------------------------------------------------------------------------
summary_manu_serv_kable
# saveRDS(summary_manu_serv_kable, paste0(output, "summary_manu_serv_kable.RDS"))
...
Nevertheless, the model performs better in predicting high growth firms in services. In this sector it exhibits a lower false positive rate and a lower false negative rate. Notably, the false positive rate in our holdout set is at zero, meaning we classified no firm in the services sector as fast growing that did not turn out to become a high growth firm. This finding points to the services industry as an attractive market albeit its lower average growth rates. It seems to be the case that the characteristics of firms in services make their future growth more predictable.

```{r include cf matrix manu, echo=FALSE, include=TRUE}
# ------------------------------------------------------------------------------
cm5_perc_kable
# saveRDS(cm5_perc_kable, paste0(output, "cm5_perc_kable.RDS"))
cm6_perc_kable
# saveRDS(cm6_perc_kable, paste0(output, "cm6_perc_kable.RDS"))
...
<!-- Task3.1: Use some methods to interpret your random forest probability model -->
...
for (i in seq_along(rf_models)) {
  ...
  current_plot_all <- plot_ly(rf_model_var_imp_df[1:30, ], x = ~imp_percentage, y = ~reorder(varname, imp), type = 'bar', orientation = 'h') %>%
    layout(
      xaxis = list(title = "Importance", tickformat = ".00%"),
...
```{r plotly,echo=FALSE, include=F, warning=F}
# ------------------------------------------------------------------------------
# Create an empty list to store the plots
...
```{r pdp,echo=FALSE, include=FALSE, warning=F, eval=FALSE}
# ------------------------------------------------------------------------------
# Partial Dependence Plots bisnode ---------------------------------------------

top10rfvar  <- c("material_exp","d1_sales_mil_log","personnel_exp","liq_assets","curr_liab","curr_assets","inc_bef_tax","share_eq","profit_loss_year","ceo_age")
```


```{r pdp2,echo=FALSE, include=FALSE, warning=F, eval=FALSE}
# ONLY DO ONCE AND STORE (bisnode)

# Create a list to store the PDP outputs
# pdp_outputs <- list()
# 
# #top10rfvar <- c("ceo_age")
# 
# # Loop over predictor variables
# for (pred_var in top10rfvar) {
#   # Generate PDP for the current predictor variable
#   pdp_output <- pdp::partial(rf_model_p, pred.var = pred_var, pred.grid = distinct_(bisnode_holdout[1:100,], pred_var), train = bisnode_work[1:100,])
#   
#   # Save the PDP output
#   pdp_outputs[[paste0("pdp_", pred_var)]] <- pdp_output
# }

#saveRDS(pdp_outputs, paste0(output, "pdp_outputs.RDS"))

```

```{r}
pdp_outputs <- readRDS(paste0(output, "pdp_outputs.RDS"))
rf_pdp_plot <- list()
# Loop over PDP outputs
for (pred_var in names(pdp_outputs)) {
  # Extract PDP output for the current predictor variable
  pdp_output <- pdp_outputs[[pred_var]]

  # Customize plot based on the type of variable (numeric or factor)
  if (is.factor(bisnode_holdout[[pred_var]])) {
    dp_plot <- pdp_output %>%
      autoplot() +
      geom_point(color = color[1], size = 2) +
      ylab("Predicited Growth") +
      xlab(pred_var) +
      theme_bg() 
    ...
    # Save the plot
  rf_pdp_plot[[paste0(pred_var)]] <- pdp_plot
  }
}
```

```{r pdp3,echo=FALSE, include=FALSE, warning=F, eval=FALSE}

# ------------------------------------------------------------------------------
# Partial Dependence Plots MANU-------------------------------------------------

# ONLY DO ONCE AND STORE (manu)

# Create a list to store the PDP outputs
# pdp_outputs_manu <- list()
# 
# #rfvars_test <- c("ceo_age","origin","")
# # Loop over predictor variables
# for (pred_var in top10rfvar) {
#   # Generate PDP for the current predictor variable
#   pdp_output <- pdp::partial(rf_model_p_manu, pred.var = pred_var, pred.grid = distinct_(manu_holdout[1:100,], pred_var), train = manu_work[1:100,])
#   
#   # Save the PDP output
#   pdp_outputs_manu[[paste0("pdp_", pred_var)]] <- pdp_output
# }

#saveRDS(pdp_outputs_manu, paste0(output, "pdp_outputs_manu.RDS"))

```

```{r}
pdp_outputs_manu <- readRDS(paste0(output, "pdp_outputs_manu.RDS"))
rf_pdp_plot_manu  <- list()
# Loop over PDP outputs
for (pred_var in names(pdp_outputs_manu)) {
  # Extract PDP output for the current predictor variable
  pdp_output <- pdp_outputs_manu[[pred_var]]

  # Customize plot based on the type of variable (numeric or factor)
  if (is.factor(manu_holdout[[pred_var]])) {
    dp_plot <- pdp_output %>%
      autoplot() +
      geom_point(color = color[1], size = 2) +
      ylab("Predicited Growth") +
    ...
    # Save the plot
  rf_pdp_plot_manu[[paste0(pred_var)]] <- pdp_plot
  }
}

#rf_pdp_plot_manu$pdp_ceo_age
```

```{r pdp4,echo=FALSE, include=FALSE, warning=F, eval=FALSE}

# ------------------------------------------------------------------------------
# Partial Dependence Plots SERV-------------------------------------------------

# ONLY DO ONCE AND STORE (serv)

# # Create a list to store the PDP outputs
# pdp_outputs_serv <- list()
# 
# #rfvars_test <- c("ceo_age","origin","")
# # Loop over predictor variables
# for (pred_var in top10rfvar) {
#   # Generate PDP for the current predictor variable
#   pdp_output <- pdp::partial(rf_model_p_serv, pred.var = pred_var, pred.grid = distinct_(serv_holdout[1:100,], pred_var), train = serv_work[1:100,])
#   
#   # Save the PDP output
#   pdp_outputs_serv[[paste0("pdp_", pred_var)]] <- pdp_output
# }
# 
# saveRDS(pdp_outputs_serv, paste0(output, "pdp_outputs_serv.RDS"))


```

```{r}
pdp_outputs_serv <- readRDS(paste0(output, "pdp_outputs_serv.RDS"))
rf_pdp_plot_serv <- list()
# Loop over PDP outputs
for (pred_var in names(pdp_outputs_serv)) {
  # Extract PDP output for the current predictor variable
  pdp_output <- pdp_outputs_serv[[pred_var]]

  # Customize plot based on the type of variable (numeric or factor)
  if (is.factor(serv_holdout[[pred_var]])) {
    dp_plot <- pdp_output %>%
      autoplot() +
      geom_point(color = color[1], size = 2) +
      ylab("Predicited Growth") +
  ...
  pdp_plot <- pdp_output %>%
      ...
      theme_bg()
    # Save the plot
  rf_pdp_plot_serv[[paste0(pred_var)]] <- pdp_plot
  }
}

#rf_pdp_plot_serv$pdp_ceo_age
```
...
```{r}
# save.image(file = "A3.RData") # saves in data_dir!

# save(list = c("bisnode", "bisnode_work","bisnode_holdout","rf_model_p","rf_model_p_manu","rf_model_p_serv","rf_model_p_InteractLossPlot","rf_model_p_manu_InteractLossPlot","rf_model_p_serv_InteractLossPlot","rf_model_plots","rf_model_plots_grouped","rfvars","pdp_outputs","pdp_outputs_manu","pdp_outputs_serv","rf_pdp_plot","rf_pdp_plot_manu","rf_pdp_plot_serv"), file = "A3_dashboard.RData")
