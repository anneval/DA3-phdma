...
# t: with fullRank = TRUE one variable is dropped to avoid multicollinearity
...
# t: don't think imputation makes sense with more than 90% missing values
bisnode <- bisnode %>%        
...
# @Teresa - maybe have that under label engineering? t: only moved it up because I use it to filter sales_mil below, but true it doesn't really fit here...
# Question Anne:   mutate(sales = ifelse(sales < 0, 1, sales), warum macht er aus sales einfach einen dummy? T: ich glaub die Zeile war weil er dann den Log nimmt und keine 0 haben wollte,
# nachdem wir aber schon davor nach status_alive gefiltert haben, brauche wir das nicht, denke ich
bisnode <- bisnode %>%
  filter(status_alive == TRUE) %>% # only look at firms that have some sales
...
bisnode <- bisnode %>%
  group_by(comp_id) %>%
  arrange(year) %>%
  mutate(sales_growth = (lead(sales)/sales -1)) %>%
  ungroup()
...
bisnode <- bisnode %>%
  group_by(comp_id) %>%
  arrange(year) %>%
  mutate(sales_growth_2y = (lead(sales, n = 2L)/sales -1)/2) %>%
  ungroup()
...
bisnode <- bisnode %>%
  group_by(comp_id) %>%
  arrange(year) %>%
  mutate(labor_growth = (lead(labor_avg)/labor_avg - 1)) %>%
  ungroup()
...
bisnode <- bisnode %>%
  group_by(comp_id) %>%
  arrange(year) %>%
  mutate(d1_labor_avg_log = lead(ln_labor_avg) - ln_labor_avg ) %>%
  ungroup()
...
bisnode <- bisnode %>%
  group_by(comp_id) %>%
  arrange(year) %>%
  mutate(birch_ind = ((lead(labor_avg) - labor_avg) * (lead(labor_avg)/labor_avg - 1)) ) %>%
  ungroup()
...
# why did we not add these? done at the beginning
...
# also ind2??
dmy <- dummyVars("~ ind + ind2_cat", data = bisnode, fullRank = TRUE)
new_dat <- data.frame(predict(dmy, newdata = bisnode))
# add new variables to data
bisnode <- cbind(bisnode, new_dat)
...
# mock data on fast growth and smaller data set (delete later)
bisnode$fast <- ifelse(bisnode$sales_growth > 
                         median(bisnode$sales_growth, na.rm = TRUE) &
                         !is.na(bisnode$sales_growth), 
                       1, 0)
bisnode$fast[is.na(bisnode$fast)] <- 0
bisnode_mock <- bisnode[sample(1:nrow(bisnode), 5000),]

# scatterplot computation
plots <- list()

# relevant numeric vars
vars_num <- c(zero, any, c("total_assets", "ceo_age", "age", "inoffice_days"))
# add sales/labour var?

# check relationships
for (var in vars_num){
  
  if(any(is.na(bisnode_mock[, var]))){
    print(paste0(var, " entails NAs")) # check for lowess
  } else{
    plots[[var]] <- ggplot(data = bisnode_mock, aes_string(x = var, y = "fast")) +
...
plots$intang_assets_rel
plots[[15]]
# ?? 1, 5, 6, 7, 8, 11
# 4, 10, 12, 13, 15

# add quadratics yes or no? all variables considered? winsorise more?
quad <- c("intang_assets_rel", "inc_bef_tax_rel", "total_assets", "ceo_age", "inoffice_days")
...
# loop did not work
ggplot(bisnode, aes(x = factor(gender), fill = factor(fast))) +
# ggplot(bisnode, aes(x = factor(ceo_old), fill = factor(fast))) +
# ggplot(bisnode, aes(x = factor(ceo_young), fill = factor(fast))) +
...
# interactions
interactions_plot <- ggplot(bisnode, aes(x = labor_avg, y = fast, color = ind2)) +
  # ggplot(bisnode, aes(x = age, y = fast, color = ind2_cat)) +
  theme_bw() + 
  geom_point() + 
  geom_point(alpha = .3, 
             size = .9) +
...
# Gabor's code did not work here
bisnode <- bisnode %>%
...
# Q: Add that already to sample design?
bisnode <- bisnode %>%
  filter(year == 2012)
```
...
rawvars <-  c("curr_assets", "curr_liab", "extra_exp", "extra_inc", "extra_profit_loss", "fixed_assets",
              ...
              "profit_loss_year", "sales", "share_eq", "subscribed_cap", "tang_assets", # why tang_asset left out in gabor's code? 
              )
...
engvar2 <- c("extra_profit_loss_rel_quad", "inc_bef_tax_rel_quad",
             "profit_loss_year_rel_quad", "share_eq_rel_quad", "intang_assets_rel_quad", "total_assets_quad", # instead total_assets_rel?
             "ceo_age_quad", "inoffice_days_quad")
...
hr <- c("female", "ceo_old", "ceo_young",
        "ceo_age",
        "flag_miss_ceo_age", "ceo_count", "labor_avg",
        "flag_miss_labor_avg", "foreign_management", "inoffice_days", "multiple_ceo")
...
# target variable
## use different ones for comparison and shiny? 
target <- c("")
