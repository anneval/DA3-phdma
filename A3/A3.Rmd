...
# make sure that high_growth is interpreted as case and low_growth as control
...
for (model_name in names(logit_models)) {
  ...
  for (fold in c("Fold1", "Fold2", "Fold3", "Fold4", "Fold5")) {
    ...
    expected_loss_cv[[fold]] <- (best_treshold$fp*FP + best_treshold$fn*FN)/length(cv_fold$dhigh_growth)
  }
...
rf_model_var_imp_plot_all <- ggplot(rf_model_var_imp_df, aes(x = reorder(varname, imp), y = imp_percentage)) +
  geom_point(color = color[1], size = 1) +
  geom_segment(aes(x = varname, xend = varname, y = 0, yend = imp_percentage), color = color[1], size = 1) +
  ylab("Importance (Percent)") +
  xlab("Variable Name") +
