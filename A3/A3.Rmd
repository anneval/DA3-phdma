...
bisnode <- bisnode %>%
  mutate(flag_missing_age = as.numeric(is.na(ceo_age)),
         ceo_age = round(ifelse(is.na(ceo_age), mean(ceo_age, na.rm = TRUE), ceo_age)),
         ceo_young = as.numeric(ceo_age < 40 & !is.na(ceo_age)),
...
bisnode <- bisnode %>%
  mutate(flag_asset_problem = ifelse(intang_assets < 0 | curr_assets < 0 | fixed_assets < 0, 1, 0)) # flag negative ones
...
bisnode <- bisnode %>%
  mutate_at(vars("intang_assets", "curr_liab", "fixed_assets", 
            ...
            funs("relative" = ifelse(total_assets == 0, 0, ./total_assets)))
