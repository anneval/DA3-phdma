...
```{r}
# maybe eaasier version load entire data insetad of just RF
...
rf_old <- read_rds(paste(data_in,"rf_A2.rds",sep = "/")) # save in data dir!!!
```
...
rf_model_var_imp <- importance(rf_old$finalModel)/1000
rf_model_var_imp_df <-
...
varnames <- rf_old$finalModel$xNames
...
# pdp_n_acc <- pdp::partial(rf_old, pred.var = "accomodates", pred.grid = distinct_(listings_holdout, "accommodates"), train = listings_holdout)
