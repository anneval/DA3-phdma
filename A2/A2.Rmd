...
Transform DKK to EUR SEE if RMSE drops, plots for results?3Ms--> 
...
#X2  <- c(
 # paste("f_property_type * (", paste(amenities, collapse = " + "), ")", sep = "")
#)
```
...
print(nrow(lasso_coeffs_nz)) # keep 61 from ????
...
#kable(x = rf_tuning_model, format = "latex", digits = 2, caption = "CV RMSE") %>%
...
result_rmse
#kable(x = result_rmse, format = "latex", digits = 3, booktabs=TRUE, linesep = "") %>%
  #cat(.,file= paste0(output,"cv_rmse.tex"))
```
```{r,echo=FALSE, include=FALSE, warning=F}
...
kable(x = result_5, format = "latex", digits = 3, booktabs=TRUE, linesep = "") %>%
  cat(.,file= paste0(output,"Comparison_models_houldout_rmse.tex"))
...
mean(data_holdout_w_prediction$predicted_price_rf) ## needed for intro 
...
Since Machine Learning methods are often seen as "black-box" models, and we often do not really know how an
actual prediction is created I want to further investigate why the Random forest model was the best performing model in all scenarios discussed above. One way to do this is by looking at the features’ contribution to predicted values on average i.e. variable importance using variance importance plots and Shapley values. Moreover, I will consider partial correlations displayed by partial dependence plots.





- a measure of how useful a variable is for prediction/ hich features contribute the most to the predictive performance.
I For each variable it captures the overall contribution to reducing RMSE
I Shows relative importance


This can be valuable in understanding what factors influence Airbnb prices the most, helping users and hosts make informed decisions.
...
#plot(varImp(rf_model)) # very 'full' graph 
```
```{r,echo=FALSE, include=FALSE, warning=F}
cutoff = 600
...
#save_fig("rf_varimp1",output, "large") # nicer output!!Appendix 
...
#dummy <- dummyVars(" ~ .", data=listings_train, fullRank=T, sep = NULL)


dummy <- dummyVars(" ~  accommodates + beds + f_property_type + f_room_type + n_days_since + flag_days_since + neighbourhood_cleansed + bathrooms + number_of_reviews + review_scores_rating + flag_review_scores_rating + accommodates2 + n_days_since2 + n_days_since3 + a_Parking + a_LongTermStay + a_Kitchen + a_Childfriendly + a_Laundry + a_OutdoorArea + a_Heating + a_Toiletry + a_TeaCoffee + a_Internet + a_Entertainment + a_Security + a_Selfcheckin + a_HomeOffice + a_Petfriendly", data=listings_work, fullRank=T, sep = NULL)

#perform one-hot encoding on data frame
data_holdout_ohe <- data.frame(predict(dummy, newdata=listings_holdout))
#listings_clean_train
...
Error: Error: Missing variables in new data: f_number_of_reviews1, f_number_of_reviews2

# Subset new data to include only variables used in the model
#new_data_subset <- data_holdout_ohe[, model_variable_names]
...
# Predict using the model and the subsetted new data
#predictions <- predict(your_rf_model, data = new_data_subset)
...
#describe(data_holdout_w_prediction$accommodates)
```


```{r,echo=FALSE, include=FALSE, warning=F}
pdp_n_roomtype <- pdp::partial(rf_model, pred.var = "f_room_type", pred.grid = distinct_(listings_holdout, "f_room_type"), train = listings_holdout)
...
Békés,G., Kézdi,G.(2021).R, Python and Stata code for Data Analysis for Business, Economics, and Policy, ch14-airbnb-reg & ch16-airbnb-random-forest  , GitHub repository, https://github.com/gabors-data-analysis/da_case_studies/tree/master/ch14-airbnb-reg
https://github.com/gabors-data-analysis/da_case_studies/tree/master/ch16-airbnb-random-forest

Békés, G., & Kézdi, G. (2021). Data analysis for business, economics, and policy. Cambridge University Press, chapter 14&16.!--> 
