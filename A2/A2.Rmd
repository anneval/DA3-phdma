...
output:
  ...
  df_print: paged
---
<!-- SOME NOTES --> # for comments later I do not want to be shown in pdf 

## clean up test/treain/hold out....
...
# Add a new column to each dataframe to identify their origin
...
#  c("Barn", "Entire cabin","Boat","Hut","Private room in boat","Dome","Farm 
...
train_indices <- as.integer(createDataPartition(listings_work$price_n, p = 0.7, list = FALSE))
listings_clean_train <- listings_work[train_indices, ]
listings_clean_holdout <- listings_work[-train_indices, ]

dim(listings_clean_train)
dim(listings_clean_holdout)# test here rather 
...
system.time({
rf_model_1 <- train(
 price_n ~ accommodates + beds + f_property_type + f_room_type + n_days_since + flag_days_since + neighbourhood_cleansed + bathrooms + number_of_reviews + review_scores_rating + flag_review_scores_rating + accommodates2 + n_days_since2 + n_days_since3 + a_Parking + a_LongTermStay + a_Kitchen + a_Childfriendly + a_Laundry + a_OutdoorArea + a_Heating + a_Toiletry + a_TeaCoffee + a_Internet + a_Entertainment + a_Security + a_Selfcheckin + a_HomeOffice + a_Petfriendly,
  data = listings_clean_train,
  method = "ranger",
  trControl = train_control,
  tuneGrid = tune_grid,
  importance = "impurity"
...
## interaction raus??
## model evaluation
## reestimate on entire work and eval (predict) on holdout
...
set.seed(1234)
system.time({
  gbm_model <- train(price_n ~ accommodates + beds + f_property_type + f_room_type + n_days_since + flag_days_since + neighbourhood_cleansed + bathrooms + number_of_reviews + review_scores_rating + flag_review_scores_rating + accommodates2 + n_days_since2 + n_days_since3 + a_Parking + a_LongTermStay + a_Kitchen + a_Childfriendly + a_Laundry + a_OutdoorArea + a_Heating + a_Toiletry + a_TeaCoffee + a_Internet + a_Entertainment + a_Security + a_Selfcheckin + a_HomeOffice + a_Petfriendly,
                     data = listings_clean_train,
                     method = "gbm",
                     trControl = train_control,
                     verbose = FALSE,
...
# predict on "new data", same as predict on hold out set 
...
result_5 <- map(final_models, ~{
  RMSE(predict(.x, newdata = listings_clean_holdout), listings_clean_holdout[["price_n"]])
}) %>% unlist() %>% as.data.frame() %>%
  rename("Comparison RMSE" = ".")

kable(x = result_5, format = "latex", digits = 3, booktabs=TRUE, linesep = "") %>%
  cat(.,file= paste0(output,"Comparison_models_houldout_rmse.tex"))


# evaluate preferred model on new data set -----------------------------

result_6 <- map(final_models, ~{
...
#########################################################################################
pdp_n_acc <- pdp::partial(rf_model_2, pred.var = "accommodates", pred.grid = distinct_(listings_holdout, "accommodates"), train = listings_train)
...
pdp_n_roomtype <- pdp::partial(rf_model_2, pred.var = "f_room_type", pred.grid = distinct_(listings_holdout, "f_room_type"), train = listings_train)
...
var_importance <- importance(rf_model_2$finalModel)
# Display the variable names and their importance scores
...
```{r}
model_variable_names <- rf_model_2$coefnames

new_data_variable_names <- colnames(data_holdout_ohe)
missing_variables <- setdiff(rf_model_2$coefnames, new_data_variable_names)
